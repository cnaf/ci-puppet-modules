class base_build_env {
  ...
  package { 'findutils': ensure => present }
  ...
  package { 'grep': ensure => present }

  package { 'gzip': ensure => present }

  package { 'info': ensure => present }
  ...
  package { 'shadow-utils': ensure => present }
  ...
  package { 'redhat-rpm-config': ensure => present }
